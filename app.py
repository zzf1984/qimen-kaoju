...
# 核心 CSS：强制手机 3x3 全景，配色方案采用老师建议的“紫暗干、蓝星干”
st.markdown("""
<style>
    .qimen-grid {
        display: grid;
    ...
    .palace-star { font-size: 12px; color: #1565C0; font-weight: bold; } /* 钢蓝色 */
    .palace-sky { font-size: 30px; color: #1A237E; font-weight: bold; }   /* 深蓝色 */
    .palace-door-row { display: flex; justify-content: space-around; align-items: center; z-index: 5; }
    .palace-door { font-size: 19px; color: #2E7D32; font-weight: bold; }   /* 深绿色 */
    .palace-hidden { font-size: 13px; color: #8E24AA; font-weight: bold; } /* 深紫色暗干 */
...
def calculate_engine(y, m, d, h, mi=0, cal_mode="公曆", method="拆補法", manual=None):
    ...
    sky_s = {p: earth_p[P_8[(P_8.index(p)-shift)%8]] for p in P_8}; sky_s[5]=earth_p[5]
    ...
    return {"lunar":lunar, "gz":[lunar.getYearInGanZhi(), lunar.getMonthInGanZhi(), gz_d, gz_t], "jq":jq_n, "ju":f"{'陽' if is_yang else '陰'}遁{ju_num}局", "earth":earth_p, "sky_s":sky_s, "sky_star":sky_star, "god":god_pan, "human":human_pan, "hidden":hidden, "shou":hx, "zf_pid":s_ref_tar, "zs_pid":door_ref_tar, "xk_pids":xk_pids, "solar":solar}
...
<div class="bazi-header">
    ...
    <div style="display:flex; justify-content:space-around; margin-top:5px;">
        ...
        <div style="text-align:center;"><div style="font-size:10px; color:grey;">時柱</div><div style="font-size:19px; font-weight:bold;">{res['gz'][3]}</div></div>
    </div>
</div>
""", unsafe_allow_html=True)
...
def get_palace_html(pid):
    ...
    g = res['god'].get(pid,"")
    s = res['sky_star'].get(pid,"")
    sky = res['sky_s'].get(pid,"")
    door = res['human'].get(pid,"")
    h = res['hidden'].get(pid,"")
    e = res['earth'].get(pid,"")
    if pid == 5: s = "天禽" if s=="" else s
    
    # 壓縮為單行 HTML，防止 Markdown 誤判

...
# 核心 CSS：支持空亡灰色、值符值使黄色背景，且强制 3x3 布局
st.markdown("""
<style>
    .qimen-grid {
        display: grid;
    ...
    .palace-box {
        border: 2px solid #4A90E2; border-radius: 10px; padding: 8px;
        height: 180px; display: flex; flex-direction: column; 
        justify-content: space-between; box-shadow: 2px 2px 10px rgba(0,0,0,0.05);
        position: relative; overflow: hidden;
    }
    /* 基础文字样式 */
    .palace-name { font-size: 11px; color: #888; text-align: right; z-index: 2; }
    .palace-god { font-size: 20px; color: #D0021B; font-weight: bold; text-align: center; z-index: 2; }
    .palace-star-row { display: flex; justify-content: space-around; align-items: center; z-index: 2; }
    .palace-star { font-size: 13px; color: #333; }
    .palace-sky { font-size: 32px; color: #4A90E2; font-weight: bold; }
    .palace-door-row { display: flex; justify-content: space-around; align-items: center; z-index: 2; }
    .palace-door { font-size: 19px; color: #417505; font-weight: bold; }
    .palace-hidden { font-size: 13px; color: #F5A623; }
    .palace-earth { font-size: 20px; color: #333; font-weight: bold; text-align: right; z-index: 2; }
...
GRID_ORDER = [3, 8, 1, 2, 4, 6, 7, 0, 5] # 对应 巽,离,坤, 震,中,兑, 艮,坎,乾
...
# 输入控件
sy = st.sidebar.number_input("年", 1, 3000, st.session_state.curr_dt.year)
sm = st.sidebar.number_input("月", 1, 12, st.session_state.curr_dt.month)
sd = st.sidebar.number_input("日", 1, 31, st.session_state.curr_dt.day)
sh = st.sidebar.number_input("時", 0, 23, st.session_state.curr_dt.hour)
smi = st.sidebar.number_input("分", 0, 59, st.session_state.curr_dt.minute)
...
<div class="bazi-header">
    <div style="font-size:12px; color:#666;">公历: {res['solar'].toFullString()} {'(真太阳时校对)' if use_solar else ''}</div>
    <div style="display:flex; justify-content:space-around; margin-top:5px;">
        <div style="text-align:center;"><div style="font-size:10px; color:grey;">年柱</div><div style="font-size:20px; font-weight:bold;">{res['gz'][0]}</div></div>
        <div style="text-align:center;"><div style="font-size:10px; color:grey;">月柱</div><div style="font-size:20px; font-weight:bold;">{res['gz'][1]}</div></div>
        <div style="text-align:center;"><div style="font-size:10px; color:grey;">日柱</div><div style="font-size:20px; font-weight:bold;">{res['gz'][2]}</div></div>
        <div style="text-align:center;"><div style="font-size:10px; color:grey;">时柱</div><div style="font-size:20px; font-weight:bold;">{res['gz'][3]}</div></div>
    </div>
</div>
""", unsafe_allow_html=True)

# 2. 局数状态条
st.info(f"✨ **{res['jq']}** | **{res['ju']}** | 旬首: **{res['shou']}** | 空亡: 時 **{res['lunar'].getTimeXunKong()}**")
...
def get_palace_html(pid):
    ...
    is_xk = pid in res['xk_pids']
    is_zf = pid == res['zf_pid']
    is_zs = pid == res['zs_pid']
    
    if is_xk: 
        bg_style = "background-color: #f2f2f2;" # 空亡全灰
    elif is_zf and is_zs:
        bg_style = "background-color: #fffde7;" # 两者皆在，全黄
    elif is_zf:
        bg_style = "background: linear-gradient(to bottom, #fffde7 50%, #ffffff 50%);" # 值符上半截黄
    elif is_zs:
        bg_style = "background: linear-gradient(to top, #fffde7 50%, #ffffff 50%);" # 值使下半截黄
    ...
    # 构建内容
    return f'<div class="palace-box" style="{bg_style}"><div class="palace-name">{PALACE_NAMES[idx]}</div><div class="palace-god">{g}</div><div class="palace-star-row"><span class="palace-star">{s}</span><span class="palace-sky">{sky}</span></div><div class="palace-door-row"><span class="palace-door">{d}</span><span class="palace-hidden">({h})</span></div><div class="palace-earth">{earth}</div></div>'

grid_html = '<div class="qimen-grid">'
for p_idx in GRID_ORDER:
    grid_html += get_palace_html(p_idx + 1)
grid_html += '</div>'

st.markdown(grid_html, unsafe_allow_html=True)
st.divider()
# ================= 5. 千年級：八字分離精確反推 (0-3000年) =================
...
with st.expander("點擊展開：八字反推設定", expanded=False):
    ...
    search_s_y = col_r1.number_input("檢索起點(公元)", 0, 3000, 960) 
    ...
    if st.button("🚀 啟動千年八字跳躍檢索", use_container_width=True):
        ...
        if first_y == -1:
            ...
            if found_bazi:
                ...
                st.error("❌ 未找到符合該組合的時間。")

# ================= 6. 奇門全要素格局考據 (支持暗干/地盤干) =================

st.header("🔎 奇門全要素大數據搜索")
st.write("設定宮位要素組合與屏蔽條件，精確定位歷史中的特定格局。")

with st.expander("點擊展開：要素篩選條件", expanded=False):
    # 使用數字直入式年份，解除 1900 限制
    sc1, sc2, sc3, sc4, sc5 = st.columns(5)
    qs_y = sc1.number_input("起年", 0, 3000, 2026, key="qs_y")
    ...
    if st.button("🔥 啟動全要素格局掃描", use_container_width=True):
        ...
        for i in range(int(qe_days)):
            ...
            for ch in range(0, 24, 2):
                qs_data = calculate_engine(curr_date.year, curr_date.month, curr_date.day, ch, 0, method=method_in)
                for pid in t_pids:
                    # 要素匹配
        ...
        if q_results:
            st.success(f"考據完成！共找到 {len(q_results)} 個匹配結果：")
...
# ================= 7. 系統結語 =================
st.markdown("---")
st.caption("奇門遁甲考據系統 · 旗艦完滿版 | 支持公元 0-3000 年 | 全球時區與真太陽時校對 | 視覺化背景輔助")
